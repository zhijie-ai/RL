...
# 主网络和beta网络的实现
# topk修正后的概率
# 和TopKReinforce_rnn.py的区别是，在训练RNN的思路时是借鉴session-based RNN的思路，增加了y的数据作为输入，与论文的实现更为接近
...
class TopKReinforce():
    ...
    def _init_graph(self):
        ...
        with tf.variable_scope('label_emb'):
            W = tf.get_variable('label_emb',[self.item_count,self.embedding_size])
            label_context = tf.nn.embedding_lookup(W,self.label)

        # 拼接用户状态和label_context作为新的用户状态
        user_state = tf.concat([outputs,label_context],axis=1)# (bs,rnn_size+emb_size)

        with tf.variable_scope('main_policy'):
            weights=tf.get_variable('item_emb_pi',[self.item_count,self.rnn_size+self.embedding_size])
            bias = tf.get_variable('bias',[self.item_count])
            self.PI =tf.add(tf.matmul(user_state,tf.transpose(weights)),bias)
            self.PI =  tf.nn.softmax(self.PI)# PI策略
            self.alpha = cascade_model(self.PI,self.topK)

        with tf.variable_scope('beta_policy'):
            weights_beta=tf.get_variable('item_emb_beta',[self.item_count,self.rnn_size+self.embedding_size])
            bias_beta = tf.get_variable('bias_beta',[self.item_count])
            self.beta = tf.add(tf.matmul(user_state,tf.transpose(weights_beta)),bias_beta)
        ...
        with tf.variable_scope('loss'):
            ...
            ce_loss_main =tf.nn.sampled_softmax_loss(
                weights,bias,label,user_state,50,num_classes=self.item_count)
            ...
            self.beta_loss = tf.reduce_mean(tf.nn.sampled_softmax_loss(
                weights_beta,bias_beta,label,user_state,50,num_classes=self.item_count))
    ...
    def plot_pi(self,pi_loss,num=10):
        ...
        plt.legend()
        # plt.show()
        plt.savefig('jpg/reinforce_top_k_pi_rnn2.jpg')
    ...
    def plot_beta(self,beta_loss,num=10):
        ...
        plt.legend()
        # plt.show()
        plt.savefig('jpg/reinforce_top_k_beta_rnn2.jpg')
