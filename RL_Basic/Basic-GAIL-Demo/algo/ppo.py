...
class PPOTrain():
    def __init__(self,Policy,Old_Policy,gamma=0.95,clip_value=0.2,c_1=1,c_2=0.01):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.gaes=tf.placeholder(dtype=tf.float32,shape=[None],name='gaes')# 泛化优势函数
        ...
        with tf.variable_scope('loss'):
            ...
            loss_clip=-tf.reduce_mean(loss_clip)# 这里应该有个负号。因为loss_clip代表的reward
            ...
            tf.summary.scalar('Value_defference',loss_vf)
    ...
    def get_summary(self,obs,actions,gaes,rewards,v_preds_next):
        return tf.get_default_session().run(self.merged,
                                            feed_dict={
                                                self.Policy.obs: obs,
                                                self.Old_Policy.obs: obs,
                                                self.actions: actions,
    ...
    def get_gaes(self,rewards,v_preds,v_preds_next):
        ...
        # calculate generative advatage estimator(lambda = 1),see ppo paper eq(11)
    ...
    def get_grad(self,obs,actions,gaes,rewards,v_preds_next):
        return tf.get_default_session().run(self.gradients,
                                            feed_dict={
                                                self.Policy.obs: obs,
                                                self.Old_Policy.obs: obs,
                                                self.actions: actions,
