...
class MADDPG():
    def __init__(self,name,layer_norm=True,nb_actions=2,nb_input=16,nb_other_action=4):
        gamma=0.999
        ...
        def critic_network(name,action_input,reuse=True):
            with tf.variable_scope(name) as scope:
                if reuse:
        ...
        # 最大化Q值,actor的目标是最大化Q值，或者是Q对a的梯度*pi对θ的梯度
        self.actor_loss=-tf.reduce_mean(
            critic_network(name+'_critic',
                           action_input=tf.concat([self.action_input,other_action_input],axis=1),
                           reuse=True)
        )
    ...
    def train_critic(self,state,action,other_action,target,sess):
        sess.run(self.critic_train,
                 {self.state_input:state,self.action_input:action,
                  self.other_action_input:other_action,self.target_Q:target})
