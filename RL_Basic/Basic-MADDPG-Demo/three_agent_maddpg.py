...
agent1_actor_target_init,agent1_actor_target_update = create_init
_update('agent1_actor','agent1_target_actor')
...
def get_agents_action(o_n,sess,noise_ratio=0):
    agent1_action=agent1_ddpg.action(state=[o_n[0]],sess=sess)+np.random.randn(2)*noise_ratio
    agent2_action = agent2_ddpg.action(state=[o_n[1]],sess=sess)+np.random.randn(2)*noise_ratio
    agent3_action = agent3_ddpg.action(state=[o_n[2]],sess=sess)+np.random.randn(2)*noise_ratio
...
def train_agent(agent_ddpg,agent_ddpg_target,agent_memory,agent_actor_target_update,
                agent_critic_target_update,sess,other_actors):
    ...
    agent_ddpg.train_critic(state=obs_batch,action=act_batch,
                            other_action=other_act_batch,target=target,sess=sess)
...
if __name__ == '__main__':
    ...
    agent_a2_op=[tf.Summary.scalar('agent'+str(i)+'_action_2',agent_a1[i]) for i in range(3)]
    ...
    reward_1000_op=[tf.Summary.scalar('agent'+str(i)+'_reward_l1000_mean',reward_100[i]) for i in range(3)]
    ...
    summary_writer = tf.summary.FileWriter('./tree_ma_summary',graph=tf.get_default_graph())
    ...
    for i in range(1000000):
        if i%1000==0:
            ...
            for agent_index in range(3):
                summary_writer.add_summary(sess.run(reward_1000_op[agent_index],
                                                    {reward_1000[agent_index]:np.mean(reward_100_list[agent_index])}),i//1000)
        ...
        for agent_index in range(3):
            ...
            summary_writer.add_summary(
                sess.run(agent_a1_op[agent_index],{agent_a1[agent_index]:a[agent_index][1]}),i
            )
            summary_writer.add_summary(sess.run(agent_a2_op[agent_index],{agent_a2[agent_index]:a[agent_index][3]}),1)
        ...
        if i%1000==0:
            saver.save(sess,'./tree_ma_weight/'+str(i)+'.ckpt')
