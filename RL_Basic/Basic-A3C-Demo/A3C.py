...
UPDATE_GLOBAL_TIER=10
GAMMA=0.9
ENTROPY_BETA = 0.001
LR_A = 0.001 # learning_rate for actor
LR_C = 0.001
GLOBAL_RUNNING_R=[]
GLOBAL_EP = 0
...
class Worker():
    ...
    def work(self):
        ...
        while not CORD.should_stop() and GLOBAL_EP<MAX_GLOBAL_EP:
            s = self.env.reset()
            ep_r = 0
            while True:
                ...
                if total_step %UPDATE_GLOBAL_TIER ==0 or done:# update global and assign to local net
                    if done:
                        v_s_ = 0 # terminal
                    else:
