...
class DQN():
    ...
    def train_Q_network(self):
        ...
        # Step 2 calculate y
        y_batch =[]
        Q_vaue_batch = self.target_Q_value.eval(feed_dict={self.state_input:next_state_batch})
        for i in range(0,BATCH_SIZE):
            done = minibatch[i][4]
            if done:
                y_batch.append(reward_batch[i])
            else:
                y_batch.append(reward_batch[i]+GAMMA*np.max(Q_vaue_batch[i]))
        ...
        #验证优势函数的维度
        print('aaaaa',self.v.eval({self.state_input:next_state_batch}).shape)
        print('bbbbbbbb',self.A.eval({self.state_input:next_state_batch}).shape)
        print('cccccccccc',self.AT.eval({self.state_input:next_state_batch}).shape)
        print('ddddddd',Q_vaue_batch.shape)
...
def main():
    ...
    print(agent.action_dim,agent.state_dim)
