...
# 如果采集的数据超过了capacity，则将数据指针重置。
# TD误差经过截断的方式，然后与alpha做指数运算，即得到capacity数组中保存的数据，
# 每个样本被采样的概率，由capacity数组中数据归一化得到。每个样本的损失函数的权重为(样本的采样概率/capacity中最小的权重)乘以-beta

'''
1.刚开始，每个样本的权重都是1，当加到129个样本的时候，开始训练。
2.训练的时候，先sample batch个样本，同时计算出每个样本对应的w(即loss的权重)，这个w的计算，在
    《强化学习精要》中还有另外一种表示。平衡刚放进去时，样本的td-error很大，随着训练的进行，td-error很小的情况。
    训练完后，根据新的td-error去更新sumtree中的priority。
'''
...
class SumTree(object):
    ...
    def __init__(self,capacity):
        ...
        self.tree = np.zeros(2*capacity-1)#这里面存的不是概率，而是权重，相当于是TD Error，归一化之后是概率
...
class Memory(object): # stored  as (s,a,r,s_) in SumTree
    ...
    def sample(self,n):
        ...
        for i in range(n):
            ...
            # if i == 0:
            #     print('np.unique',np.unique(self.tree.tree[-self.tree.capacity:]))
        ...
        # print('total_p:{},'
        #       'max_p:{},sum:{},unique:{}'.format(
        #                                          self.tree.total_p,
        #                                          np.max(self.tree.tree[-REPLAY_SIZE:]),
        #                                          np.sum(self.tree.tree[-REPLAY_SIZE:]),
        #                                          np.unique(self.tree.tree[-REPLAY_SIZE:])))
        return b_idx,b_momery,ISWeights

    def batch_update(self,tree_idx,abs_erros):#data原本存的是td-error，这里做了一点改变。
...
class DQN():
    ...
    def create_training_method(self):
        self.action_input = tf.placeholder('float',[None,self.action_dim]) # open hot persentation
    ...
    def perceive(self,state,action,reward,next_state,done):
        ...
        self.replay_total +=1
        # print('replay_total', self.replay_total,np.unique(self.memory.tree.tree[-REPLAY_SIZE:]))
        if self.replay_total > BATCH_SIZE:
            self.train_Q_network()

        if self.replay_total == 129:
            print('====================')
            print(len([d for d in self.memory.tree.tree[-REPLAY_SIZE:] if d >0]))
            print(self.memory.tree.tree[0])

        # print('replay_total:{},total_p:{},'
        #       'max_p:{},sum:{},unique:{}'.format(self.replay_total,
        #                                         self.memory.tree.total_p,
        #                                         np.max(self.memory.tree.tree[-REPLAY_SIZE:]),
        #                                         np.sum(self.memory.tree.tree[-REPLAY_SIZE:]),
        #                                         np.unique(self.memory.tree.tree[-REPLAY_SIZE:])))
    ...
    for episode in range(EPISODE):
        ...
        if episode %100 == 0:
            ...
            for i in range(TEST):
                ...
                for j in range(STEP):
                    env.render()
                    action = agent.action(state) # direct for test
