...
AVAILABLE_CHOICES_NUMBER = len(AVAILABLE_CHOICES)
MAX_ROUND_NUMBER=10

class State(object):
    '''
    蒙特卡洛树搜索的游戏状态，记录在某一个Node节点下的状态数据，包含当前的游戏得分、当前的游戏round数、从开始到当前的游戏记录。
    ...
    def get_current_round_index(self):
        return self.curent_round_index

    def set_cuurent_round_index(self,turn):
        self.curent_round_index = turn
    ...
    def is_terminal(self):
        return self.get_current_round_index() == MAX_ROUND_NUMBER
    ...
    def get_next_state_with_random_choice(self):
        ...
        next_state.set_cuurent_round_index(self.curent_round_index+1)
...
class Node():
    ...
    def is_all_expand(self):
        return len(self.children) == AVAILABLE_CHOICES_NUMBER
...
def tree_policy(node):
    """
    蒙特卡洛树搜索的selection和expand阶段，传入当前需要开始搜索的节点(例如根节点),
        根据exploration和exploitation算法返回最好的expand节点，注意输入的节点应该不是子节点，而且是有
        未执行的Action可以expand的。
...
def expand(node):
    """
    输入一个节点，在该节点上拓展一个新的节点，使用random方法执行Action，
        返回新增的节点。注意，需要保证新增的节点与其他节点Action不同。
    :param node:
    :return:
    """
    ...
    # Check util get the new state which has differnt action from others
...
def monte_carlo_tree_search(node):
    ...
    for i in range(computation_budget):
        ...
        # 3. Update all passing node with reward
...
def main():
    # Create the initialize state and initialize node
    init_state= State()
    init_node = Node()
    ...
    print('AAAA',len(init_node.children))
