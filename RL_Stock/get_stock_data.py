...
class Downloader(object):
    ...
    def run(self):
        ...
        for index, row in stock_df.iterrows():
            ...
            if row["code"] in ["sh.600084"]:#*ST中葡
                continue
            print('processing {} {}'.format(row["code"],row["code_name"]))
            df_code = bs.query_history_k_data_plus(row["code"], self.fields,
                                                   start_date=self.date_start,
                                                   end_date=self.date_end).get_data()

            df_code.to_csv('{}/{}.{}.csv'.format(self.output_dir,row["code"],row["code_name"]), index=False)
        self.exit()
