...
def main(opts):
    ...
    '''
    session_new_index:userid
    item_new_index:itemid
    '''
    ...
    for user in xrange(size_user):
        ...
        split_tag = list(data_u['tr_val_tst'])[0]#根据该字段切分训练集，测试集和验证集
        if split_tag == 0:
        ...
        time_set = np.array(list(set(data_u['Time'])))#该用户的行为次数
        time_set.sort()

        true_t = 0
        for t in xrange(len(time_set)):
            display_set = data_u_time.get_group(time_set[t])#该用户在该时间段的行为次数
            event_cnt += 1
            sum_length += len(display_set)

            data_behavior[user][1].append(list(display_set['item_new_index']))#该时间段展示的item
            data_behavior[user][2].append(int(display_set[display_set.is_click==1]['item_new_index']))#同时间段点击的item
