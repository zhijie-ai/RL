...
city_recall_path = project_root_path+'/GAN_RL/yjp/data/sqls/sku.sql'
...
def train(hostname=None,port=None):
    ...
    if hostname and port and city_recall_sql:
        ...
        print(data.info())
        data.to_csv(root_path + '/GAN_RL/yjp/data/sku.csv',index=False)
    else:# 本地测试
        data = ld.get_data_from_csv(root_path + '/GAN_RL/yjp/data/sku.csv')
