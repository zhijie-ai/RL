...
data_user = raw_data.groupby(by='session_new_index')# 按用户分组
...
for user in range(size_user):
    ...
    split_tag = list(data_u['tr_val_tst'])[0]#tr_val_tstn:区分训练、测试、验证用户
    if split_tag == 0:
    ...
    for t in range(len(time_set)):
        display_set = data_u_time.get_group(time_set[t])# 取出同一个时间内的数据为一个event
        event_cnt += 1
